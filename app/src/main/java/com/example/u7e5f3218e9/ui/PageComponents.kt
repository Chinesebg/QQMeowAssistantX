...
internal fun MiaoPage(
    ...
    val scrollConnection = remember {
        object : NestedScrollConnection {
            override fun onPostScroll(consumed: Offset, available: Offset, source: NestedScrollSource): Offset {
                if (available.y < -1f) currentOnScrollDown(true)
                else if (available.y > 1f) currentOnScrollDown(false)
                return Offset.Zero
            }
        }
    }
    ...
    Scaffold(
        containerColor = if (transparentBackground) Color.Transparent else MiuixTheme.colorScheme.surface,
        topBar = {
